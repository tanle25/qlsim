...
<div class="-mx-4 sm:-mx-8 py-4 overflow-x-auto px-4">
    <div class="inline-block w-full shadow-md rounded-lg overflow-hidden pb-5">
        <table id="product-table" class="w-full leading-normal dark:text-gray-400">
            ...
            <tbody>
                @foreach ($partnerpackages as $pakage )
                <tr>
                    ...
                    <td>
                        <p class="text-color whitespace-no-wrap">{{__(config('constrain.pakage.'.$pakage->package->type.'.text')) }}</p>
                    </td>
                    <td>
                        <p class="text-color whitespace-no-wrap">{{ number_format($pakage->package->rent_price) }}</p>
                        {{-- @dd($simCard->partner->name) --}}
                    </td>
                    <td>
                        <p class="text-color whitespace-no-wrap">
                            {{number_format($pakage->lease_price) }}
                        </p>
                    </td>
...
<form id="package-form" action="{{url('dealer/them-goi-cuoc')}}" method="post">
    @csrf
        ...
        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 text-center">
                        ...
                        <select name="package_id" id="" class="input-field w-full leading-5">
