                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-right">
                                <button type="button" class="inline-block text-gray-500 hover:text-gray-700 btn-dropdown" data-dropdown-toggle="dropdown" data-dropdown-placement="left" data-request="{{$req->id}}"
                                >
...
<div id="dropdown" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700">
    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefault">
      <li id="btn-edit-request" data-modal-toggle="edit-request-modal">
...
<form action="{{url('update-wifi-request')}}" method="post">
    @csrf
    <div id="edit-request-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 md:inset-0 h-modal md:h-full">
        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-toggle="edit-request-modal">
                    <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                    <span class="sr-only">Close modal</span>
                </button>
                <div class="p-6 text-center">
                    {{-- <svg aria-hidden="true" class="mx-auto mb-4 w-14 h-14 text-gray-400 dark:text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> --}}
                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">{{__('Edit :resource',['resource'=>__('request wifi')])}}</h3>
                    <div class="modal-body">
                        <input type="hidden" name="id" id="request-id">
                        <select name="status" class="input-field mb-3" id="request-status">
                            <option value="1">{{__('Done')}}</option>
                            <option value="0">{{__('new')}}</option>
                        </select>
                        <textarea id="request-message" name="content" rows="4" class="block mb-3 p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Your message..."></textarea>
                    </div>
                    ...
                    <div class="flex justify-center">
                        ...
                        <button data-modal-toggle="edit-request-modal" type="button" class="light-btn">{{__('Cancel')}}</button>
                    </div>
...
const dropdown = new Dropdown(targetEl, triggerEl);
    ...
    $(document).ready( function () {
        ...
        $('#btn-edit-request').click(function(){
            $.ajax({
                type: "get",
                url: "{{url('get-wifi-request')}}/"+requestId,
                dataType: "json",
                success: function (response) {
                    $('#request-id').val(response.id);
                    $('#request-status').val(response.status);
                    $('#request-message').text(response.content)
                }
            });
        });
